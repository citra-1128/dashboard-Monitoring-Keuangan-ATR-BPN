...
# Koefisien regresi dari laporan SPSS (X=indeks bulan, Y=% non-kumulatif)
REGRESI = {
...
def get_pct_bulanan(df, seksi, tahun):
    """
    % realisasi non-kumulatif per bulan.
    TotalReal di data adalah kumulatif → ambil selisih antar bulan.
    Jika selisih negatif (koreksi/revisi) → set 0.
    """
...
def get_sisa(df, seksi, tahun, sampai_idx):
    ...
    for i in range(sampai_idx, -1, -1):
        ...
        if not sub.empty:
            ...
            break
    # Cap realisasi ke pagu (anomali data)
...
def compute_all_regresi(_df):
    """
    Regresi per seksi:
      X = indeks bulan (1=Feb..11=Des)
      Y = % realisasi non-kumulatif
      Data di-pool dari semua tahun 2018-2025 → 88 obs per seksi
    Proyeksi 2026: substitusi X=1..11 ke persamaan 2018-2025
    Proyeksi 2027: fit ulang 2018-2026 (tambah 11 obs proj2026)
    """
    tahun_aktual = list(range(2018, 2026))

    def regresi_manual(x_vals, y_vals):
    ...
    for s in SEKSI_LIST:
        ...
        a1, b1, r2_1, mae1, y_pred_pool = regresi_manual(x_pool, y_pool)

        # Proyeksi 2026: X=1..11
        proj2026 = [max(0.0, round(a1 + b1*x, 4)) for x in range(1, 12)]

        # Tahap 2: pool 2018-2025 + proj2026
        x_pool2 = x_pool.copy()
        y_pool2 = y_pool.copy()
        ...
        a2, b2, r2_2, mae2, y_pred_pool2 = regresi_manual(x_pool2, y_pool2)

        # Proyeksi 2027: X=1..11
        proj2027 = [max(0.0, round(a2 + b2*x, 4)) for x in range(1, 12)]

        hasil[s] = {
            'a1': a1, 'b1': b1, 'r2_1': r2_1, 'mae1': mae1,
            'x_pool': x_pool, 'y_pool': y_pool, 'y_pred_pool': y_pred_pool,
            'proj2026': proj2026,
            'a2': a2, 'b2': b2, 'r2_2': r2_2, 'mae2': mae2,
            'x_pool2': x_pool2, 'y_pool2': y_pool2, 'y_pred_pool2': y_pred_pool2,
            'proj2027': proj2027,
        }
    return hasil
...
if halaman == "🏠 Dashboard":
    ...
    with col1:
        ...
        if ROLE == 'keuangan':
            pct_gauge = summary['pct_realisasi']
        else:
            pct_gauge = summary['per_seksi'][SEKSI_AKSES]['realisasi_pct']
...
# ═══════════════════════════════════════════════════════════════════════════════
# HALAMAN 3 — ANALISIS & PROYEKSI (hanya keuangan)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_risiko:
        ...
        st.markdown("**📐 Ringkasan Hasil Regresi per Seksi (dari laporan)**")
        rows_reg = []
        for s in SEKSI_LIST:
            r = REGRESI[s]
            rows_reg.append({
...
# ═══════════════════════════════════════════════════════════════════════════════
# HALAMAN 4 — REGRESI LINEAR (hanya keuangan)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("📖 Tentang Metode Regresi Linear", expanded=False):
        ...
        • Data di-<i>pool</i> dari 2018–2025 → <b>88 observasi per seksi</b><br>
        ...
        <b>Tahap 1:</b> Pool 2018–2025 → fit → persamaan → substitusi X=1..11 → <b>Proyeksi 2026</b><br>
        <b>Tahap 2:</b> Pool 2018–2025 + Proyeksi 2026 → fit ulang → substitusi X=1..11 → <b>Proyeksi 2027</b>
        </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab_r1:
        ...
        <b>📌 Regresi Tahap 1</b> — Data aktual 2018–2025 di-<i>pool</i>. <b>X = indeks bulan</b> (1=Feb…11=Des),
        <b>Y = % realisasi non-kumulatif</b>. 1 persamaan per seksi. Proyeksi 2026 = substitusi X=1..11.
        </div>
        """, unsafe_allow_html=True)
        ...
        <b>Persamaan:</b> &nbsp; Ŷ = {a1v} + ({b1v}) · X &nbsp;|&nbsp;
        X = 1 (Feb) … 11 (Des) &nbsp;|&nbsp; n = {len(rd1['x_pool'])} observasi
        </div>
        """, unsafe_allow_html=True)

        fig_r1 = go.Figure()
        fig_r1.add_trace(go.Scatter(
            x=rd1['x_pool'], y=rd1['y_pool'], mode='markers',
        ...
        fig_r1.update_layout(
            title=f"Scatter & Regresi Tahap 1 — {sel_s1} ({SEKSI_NAMA[sel_s1]})",
    ...
    with tab_r2:
        ...
        <b>📌 Regresi Tahap 2</b> — Data 2018–2025 (aktual) + 2026 (proyeksi) di-<i>pool</i>.
        Fit ulang → persamaan baru → substitusi X=1..11 → <b>Proyeksi 2027</b>.
        </div>
        """, unsafe_allow_html=True)
        ...
        n = {len(rd2['x_pool2'])} observasi (2018–2026)
        </div>
        """, unsafe_allow_html=True)

        fig_r2 = go.Figure()
        n_aktual = len(rd2['x_pool2']) - 11
        fig_r2.add_trace(go.Scatter(
            x=rd2['x_pool2'][:n_aktual], y=rd2['y_pool2'][:n_aktual], mode='markers',
            name='Data Aktual (2018–2025)',
            marker=dict(size=6,color='#1a3a6b',opacity=0.5)
        ))
        fig_r2.add_trace(go.Scatter(
        ...
        fig_r2.update_layout(
            title=f"Scatter & Regresi Tahap 2 — {sel_s2} ({SEKSI_NAMA[sel_s2]})",
            xaxis=dict(title='Indeks Bulan (1=Feb … 11=Des)',gridcolor='#f0f0f0',
                       tickvals=list(range(1,12)),ticktext=BULAN),
            yaxis=dict(title='% Realisasi Non-Kumulatif',gridcolor='#f0f0f0'),
            legend=dict(orientation='h',y=-0.3),
